  ...
  try {
    ...
    internal.services.forEach(function (service) {
      if (type.isWildcard() || type.matches(service.serviceType)) {
        if (self.doNotRespondForServices.indexOf(service.alias) === -1) {
          debug('answering query');
          self.networking.send(pf.buildANPacket.apply(self, [DNSRecord.TTL]));
        }
        else {
          debug('skipping query; answers already contain the service %s',
                service.alias);
        }
      } else {
...
// reduction function
internal.handleAnswer = function (doNotRespondForServices, rec) {
  try {
    ...
    if ((!internal.probes.length) && (rec.type === DNSRecord.Type.PTR)) {
      internal.services.forEach(function (service) {
        if (rec.data === service.alias) {
          doNotRespondForServices.push(service.alias);
        }
      });
    }
  } catch (err) {
  ...
  return doNotRespondForServices;
};
...
var Advertisement = module.exports = function (
  ...
  this.doNotRespondForServices = [];

  networking.on('packets', function (packets /*, remote, connection*/) {
    packets.forEach(function (packet) {
      self.doNotRespondForServices =
        packet.answer.reduce(internal.handleAnswer.bind(self), []);
