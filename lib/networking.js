...
Networking.prototype.start = function () {
  ...
  for (var key in interfaces) {
    if (interfaces.hasOwnProperty(key)) {
      for (var i = 0; i < interfaces[key].length; i++) {
        ...
        to_create.push({interfaceIndex:index++,
  ...
  if(to_create.length > 1) {
    to_create.forEach( function (tc) {
      this.createSenderSocket(tc.interfaceIndex, tc.networkInterface,
          tc.address, 5353, self.bindToAddress.bind(this));
    });
  }
  
  this.createBindAnySocket(5353, this.bindToAddress.bind(this), (to_create.length > 1) ? false: true);
};
...
Networking.prototype.send = function (packet) {
  ...
  function onEach(connection) {
    ...
    var ip_address = sock.address().address;
