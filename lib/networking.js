...
function unusedPort(cb) {
    ...
    server.on('error', function (err) { unusedPort(cb) })
};

unusedPort(function(newPort) {
    port = newPort;
});
  ...
  if (toCreate.length > 1) {
    ...
    toCreate.forEach(function (tc) {
      self.createSenderSocket(tc.interfaceIndex, tc.networkInterface,
          tc.address, port, self.bindToAddress.bind(self));
    });
  }

  this.createBindAnySocket(port,
...
Networking.prototype.send = function (packet) {
  ...
  function onEach(connection) {
    ...
    sock.send(buf, 0, buf.length, port, MDNS_MULTICAST, function (err, bytes) {
